...
class TokenParser
{
    ...
    /**
     * Peeks at a token at a certain offset from the current position
     *
     * @param integer $offset
     * @return array|string
     */
    private function peekToken($offset)
    {
        $pos = $this->ptr + $offset;
        $increment = $offset < 0 ? -1 : +1;
        while ($pos >= 0 && $pos < $this->numTokens) {
        ...
        throw new \OutOfRangeException('Tried to peek at out-of-range token at position ' . $pos);
    }
     ...
     *
     * @param string[]|int[] $tokenTypes
     *            Array of single characters or token types to stop on.
    ...
    private function parseClass()
    {
        ...
        $previousToken = $this->peekToken(-1);
        if ($previousToken[0] === T_DOUBLE_COLON) {
            // This was a reference to a class name (TokenParser::class)
            return null;
        }
        $class = '';
        $token = $this->nextUntil([
